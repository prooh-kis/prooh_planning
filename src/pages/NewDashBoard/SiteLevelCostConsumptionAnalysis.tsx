...
export const SiteLevelCostConsumptionAnalysis = ({
  ...
  const getCostConsumptionData = () => {
    const datesArray = tabWiseSiteData?.costData && Object.keys(tabWiseSiteData?.costData)?.length !== 0 ? Object.keys(tabWiseSiteData?.costData)?.map((date: any) => date) : [];
    const countsArray = tabWiseSiteData?.costData && Object.values(tabWiseSiteData?.costData)?.length !== 0 ? Object.values(tabWiseSiteData?.costData)?.map((slot: any) => slot) : [];
      ...
      dayTypes: {
        ...
        costConsumption:
          ...
          type == "dayType" && checked && value == "all" ? 
            [] :
          type == "dayType" && !checked ?
            dayTimeFilters?.dayTypes?.costConsumption?.filter((f: any) => f !== value) :
      ...
      timezones: {
        ...
        costConsumption:
          ...
          type == "timezone" && checked && value == "all" ? 
            [] :
          type == "timezone" && !checked ?
            dayTimeFilters?.timezones?.costConsumption?.filter((f: any) => f !== value) :
  ...
  useEffect(() => {
    if (
      tabWiseSiteData && 
      (dayTimeFilters?.dayTypes?.costConsumption?.length === 0 || !dayTimeFilters?.dayTypes?.costConsumption) &&
      (dayTimeFilters?.timezones?.costConsumption?.length === 0 || !dayTimeFilters?.timezones?.costConsumption)
    ) {
      setDayTimeFilters((prev: any) => ({
        ...prev,
        dayTypes: {
  ...
  useEffect(() => {
    ...
    dispatch(getSiteLevelPerformanceTabWiseForPlannerDashboard({
      ...
      dayTypes: dayTimeFilters.dayTypes.costConsumption?.filter((f: any) => f !== "all"),
      timezones: dayTimeFilters.timezones.costConsumption?.filter((f: any) => f !== "all"),
    }));
                      ...
                      <div className="flex items-center w-full gap-2">
                        ...
                        <h1 className="text-[10px] col-span-1">
                          {dayTimePercentView[1] ? `${(tabWiseSiteData?.dayWiseData[
                                dayKey
                              ]?.costConsumed*100/tabWiseSiteData?.dayWiseData[
                                dayKey
                              ]?.costPromised).toFixed(0)}%` : formatNumber(tabWiseSiteData?.dayWiseData[
                                dayKey
                              ]?.costConsumed)
                          }
                        </h1>
                      ...
                      <div className="flex items-center w-full gap-2">
                        ...
                        <h1 className="text-[10px] col-span-1">
                          {dayTimePercentView[1] ? `${(tabWiseSiteData?.timeWiseData[
                              timeKey
                            ]?.costConsumed*100/tabWiseSiteData?.timeWiseData[
                              timeKey
                            ]?.costPromised).toFixed(0)}%` : formatNumber(tabWiseSiteData?.timeWiseData[
                              timeKey
                            ]?.costConsumed)
                          }
                        </h1>
