...
export const ScreenSummaryTable = ({
  ...
  const handleData = useCallback(
    (myData: any) => {
          ...
          Object.keys(myData[city][tp]).forEach((st) => {
            ...
            Object.keys(myData[city][tp][st]).forEach((zone) => {
              ...
              myData[city][tp][st][zone].forEach((screen: any) => {
                ...
                const localScreenData = getDataFromLocalStorage(
                  SCREEN_SUMMARY_SELECTION
      ...
      setScreensBuyingCount(
        storedSelection && Object.keys(storedSelection).length
          ? storedSelection
          : screens
      );
    },
  ...
  const handleScreenTypeClick = useCallback(
    (screenType: string, tp: string) => {
      const screens = Object.values(
        data?.[currentCity]?.[tp]?.[screenType] || {}
      ).flat() as any[];

      const allSelected = screens.every(
        (s) => screensBuyingCount[currentCity]?.[s._id]?.status
      );
      ...
      <div className="w-full h-full border-b">
        <div className="bg-[#f7f7f7] grid grid-cols-12 items-center rounded-t text-[#6f7f8e]">
          <div className="py-2 col-span-2">
            <h1 className="text-[16px] font-bold text-center">Touch points</h1>
          </div>
              ...
              <div
                className={`grid ${
                  Object.keys(cityZones[currentCity]).length > 3
                    ...
                    : "grid-cols-[repeat(auto-fit,minmax(6rem,0.5fr))]"
                } gap-0`}
                style={{
                  width:
                    Object.keys(cityZones[currentCity]).length > 3
                    ...
                    <div className="cursor-pointer col-span-2 py-2 px-4 border-r">
                      <div className="flex gap-2 items-center">
                        <Checkbox
                          ...
                          checked={screenTypeToggle?.[currentCity]?.[tp]?.[st]}
                        />
                        <Tooltip title={st}>
