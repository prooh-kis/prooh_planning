...
export const EditCreativesForCampaigns = (props: any) => {
  ...
  useEffect(() => {
    if (screenData) {
      ...
      setSelectedRatio(
        screenData[Object.keys(screenData)[0]][0].screenResolution
      );
    }
  }, [screenData]);
  useEffect(() => {
            ...
            <div className="grid grid-cols-12 gap-2 p-2">
              <div className="col-span-8 rounded border border-gray-100 grid grid-cols-12">
                <div className="col-span-4 p-2">
                  <h1 className="text-[14px] font-semibold">Filters</h1>
                  <div className="p-1">
                        ...
                        {screenData[city]?.map((screen: any, j: any) => (
                          <div key={j} className="p-1">
                            <CheckboxInput
                              ...
                              textSize="12px"
                              checked={
                                selectedRatio === screen.screenResolution
                              }
                              onChange={() => {
                                setSelectedRatio(screen.screenResolution);
                              }}
                            />
                          </div>
                        ))}
                ...
                <div className="col-span-8 p-2 border-l border-gray-100">
                  <div className="flex items-center justify-between">
                    ...
                    <h1 className="text-[12px] text-gray-400 truncate">
                      Creative Ratio:{" "}
                      {selectedRatio ? selectedRatio : "None selected"}
                    </h1>
                  </div>
                  <div className="py-2">
                    <TabWithoutIcon
              ...
              <div className="col-span-4 p-2 rounded border border-gray-100">
                ...
                {screenData[selectedCity]
                  ?.filter((cr: any) => cr.screenResolution === selectedRatio)
                  ?.map((creative: any, i: any) => (
                    <div key={i}>
                      <h1>{i}</h1>
                    </div>
                  ))}
