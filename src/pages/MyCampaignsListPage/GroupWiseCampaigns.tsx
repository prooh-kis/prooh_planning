    ...
    <div className="bg-[#FFFFFF] rounded-lg p-2">
      ...
      {agencyList.length === 0 ? (
        ...
        <div className="space-y-2 h-[30vh] overflow-y-auto no-scrollbar">
                ...
                <div className="flex items-center justify-between truncate">
                  <div className="flex items-center gap-2 truncate">
                    <input
                      ...
                      checked={group === "All" ? filters?.agency.length === initialFilters?.agency.length : filters.agency.includes(group)}
                      onChange={(e) => handleFilters("agency", group, e.target.checked)}
                  ...
                  <div className="flex items-center space-x-2">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${(groupWiseCampaigns[group]?.performance || 0) > 1 ? "text-[#4DB37E]" : "text-[#EF4444]"}`}>
          ...
          {initialFilters?.agency?.length > 0 && initialFilters.agency?.filter((u: any) => !agencyList.includes(u)).map((group: any, i: any) => (
            <div key={i} className="overflow-hidden">
              <div 
