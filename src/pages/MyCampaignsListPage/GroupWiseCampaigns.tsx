...
type GroupWiseCampaignsProps = {
  title: string;
  groupWiseCampaigns: any; // Required prop for backward compatibility
};

export const GroupWiseCampaigns: React.FC<GroupWiseCampaignsProps> = ({
  title,
  groupWiseCampaigns,
}) => {
          ...
          {agencyList?.map((group: any, i: any) => (
            <div key={i} className="border border-[#E5E7EB] rounded-lg overflow-hidden">
              <div 
                ...
                onClick={() => toggleAgency(group)}
              >
                <div className="flex flex-col items-start justify-between">
                  <div className="flex items-center">
                    {expandedAgencies[group] ? (
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="flex gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-[#ECFDF5] text-[#065F46] border border-[#A7F3D0]">
                      ...
                      {groupWiseCampaigns[group].campaigns.length}
                    </span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${(groupWiseCampaigns[group]?.performance || 0) > 1 ? "bg-[#4DB37E30] text-[#4DB37E]" : "bg-[#EF444430] text-[#EF4444]"}`}>
                      {((groupWiseCampaigns[group]?.performance || 0) * 100)?.toFixed(1) || 0}%
                    </span>
                  </div>
                </div>
              </div>
              {expandedAgencies[group] && groupWiseCampaigns[group].campaigns.length > 0 && (
                <div className="divide-y divide-[#E5E7EB]">
                  {groupWiseCampaigns[group].campaigns.map((campaign: any, i: any) => (
