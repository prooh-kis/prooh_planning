...
function Screen({ screens, screen, currentSummaryTab, handleScreenClick }: any) {
  return (
    <div className={`hover:shadow-md rounded-md ${screens[Object.keys(screens)[Number(currentSummaryTab) - 1]][screen._id].status === false ? "border border-red-300" : ""}`}>
      ...
      <div className="py-2 px-1 truncate">
        ...
        <div className="flex justify-between">
          ...
          >
            ...
            {screens[Object.keys(screens)[Number(currentSummaryTab) - 1]][screen._id].status === true ? (
...
export function ViewPlanPic({ screensBuyingCount, setScreensBuyingCount, refreshScreenSummary, screenTypes, currentSummaryTab, setCurrentCity, priceFilter, cityZones, cityTP }: any) {
  ...
  const filteredScreensData = useCallback(() => {
    ...
    let allResult: any;
    let result = Object.values(screensBuyingCount[Object.keys(screensBuyingCount)[Number(currentSummaryTab) - 1]])?.map(
      (f: any) => f.data
    );
    ...
    <div className="grid grid-cols-12 gap-8 py-2">
      <div className="col-span-3 border rounded-[12px] p-3 overflow-y-auto">
        ...
        <div className="py-2">
          ...
          {Object.keys(cityZones[Object.keys(screensBuyingCount)[Number(currentSummaryTab) - 1]])?.map((zone: any, i: any) => (
            <div key={i} className="flex items-center justify-between p-2">
              ...
              />
              <p className="text-[14px]">({filteredScreensData()?.allResult?.filter((s: any) => s.location.zoneOrRegion === zone)?.length})</p>
            </div>
          ))}
        </div>
        <div className="py-2">
          <h1 className="font-semibold">Touchpoint</h1>
          {Object.keys(cityTP[Object.keys(screensBuyingCount)[Number(currentSummaryTab) - 1]])?.map((tp: any, j: any) => (
            <div key={j} className="flex items-center justify-between p-2">
              ...
              />
              <p className="text-[14px]">({filteredScreensData()?.allResult?.filter((s: any) => s.location.touchPoint === tp)?.length})</p>
        ...
        <div className="py-2">
          ...
          {Object.keys(screenTypes[Object.keys(screensBuyingCount)[Number(currentSummaryTab) - 1]])?.map((st: any, k: any) => (
            <div key={k} className="flex items-center justify-between p-2">
              ...
              />
              <p className="text-[14px]">({filteredScreensData()?.allResult?.filter((s: any) => s.screenType === st)?.length})</p>
            </div>
          ))}
        </div>
      </div>
      <div className="col-span-9 rounded grid grid-cols-12 flex flex-wrap gap-4 overflow-scroll h-[60vh]">
        {filteredScreensData()?.result?.filter((sc: any) => {
          return sc.pricePerSlot >= priceFilter?.min && sc.pricePerSlot <= priceFilter?.max
        })?.map((screen: any, index: any) => (
          <div key={index} className="col-span-3">
            <Screen handleScreenClick={handleScreenClick} screen={screen} screens={screensBuyingCount} currentSummaryTab={currentSummaryTab} />
          </div>
        ))}
      </div>  
    </div>
