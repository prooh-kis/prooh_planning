      ...
      <div className="flex items-center justify-between pt-2 mb-6">
        <div className="flex-1 h-1 bg-gray-200 relative mx-4">
          <div className="absolute inset-x-0 flex justify-between">
            ...
            />
            {[...Array(steps)].map((_, i) => (
              <div key={i} onClick={() => handleStepClick(i)} className="relative flex flex-col items-center">
                <div
                ...
                >
                  <Tooltip title={stepLabels[i]}>
                    <div className="relative mt-[-32px] w-full">
                      <div
                        className={`flex w-full gap-2 ${i + 1 <= step ? "text-primaryButton" : "text-[#D6D2D2]"}`}
                      >
                        {icons[i]}
                      </div>
                    </div>
                ...
                {i + 1 === step && (
                  <div
                    className={`absolute top-full mt-2 text-primaryButton text-[14px] font-medium whitespace-nowrap
                      ${i === 0 ? "left-0" : i + 1 === steps ? "right-0" : "left-1/2 transform -translate-x-1/2"}
                    `}
                  >
                    {stepLabels[step - 1]}
                  </div>
