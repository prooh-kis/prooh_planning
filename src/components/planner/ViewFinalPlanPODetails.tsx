...
import { CAMPAIGN, FULL_CAMPAIGN_PLAN, SCREEN_SUMMARY_TABLE_DATA, SELECTED_AUDIENCE_TOUCHPOINTS, SELECTED_SCREENS_ID, SELECTED_TRIGGER } from "../../constants/localStorageConstants";
import { formatNumber } from "../../utils/formatValue";
import { addDetailsToCreateCampaign } from "../../actions/campaignAction";
import { generatePdfFromJSON, generatePdfFromWebPage } from "../../utils/generatePdf";
...
export const ViewFinalPlanPODetails = ({
  ...
  const [poInput, setPoInput] = useState<any>({
    ...
    totalImpression: getDataFromLocalStorage(SCREEN_SUMMARY_TABLE_DATA)?.["total"]?.totalImpression || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).totalImpression,
    totalReach: getDataFromLocalStorage(SCREEN_SUMMARY_TABLE_DATA)?.["total"]?.totalReach || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).totalReach,
    screenIds: getDataFromLocalStorage(SELECTED_SCREENS_ID) || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).screenIds,
    totalCpm: getDataFromLocalStorage(SCREEN_SUMMARY_TABLE_DATA)?.["total"]?.totalCpm || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).totalCpm,
    triggers: getDataFromLocalStorage(SELECTED_TRIGGER) || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).selectedTriggers,
    totalCampaignBudget: getDataFromLocalStorage(SCREEN_SUMMARY_TABLE_DATA)?.["total"]?.totalCampaignBudget || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).totalCampaignBudget,
    cohorts: getDataFromLocalStorage(SELECTED_AUDIENCE_TOUCHPOINTS)?.cohorts || getDataFromLocalStorage(FULL_CAMPAIGN_PLAN).cohorts,
  });
  ...
  return (
    <div className="w-full py-3">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div ref={pageRef} className="col-span-1 mt-4 px-8 py-4 border border-1 border-#C3C3C3 rounded-2xl w-full">
        ...
        <div ref={pageRef} className="col-span-1 mt-4 p-8 border border-1 border-#C3C3C3 rounded-2xl w-full">
              ...
              <input title="approach" type="checkbox"
                onChange={() => {
                  ...
                  pdfToDownload["approach"] = {
                    ...
                    pageRefs: [marketRef, audienceRef, touchpointRef, locationProximityRef, poiProximityRef, mapViewRef],
          ...
          <button
            ...
            onClick={() => {
              ...
              //   generatePdfFromJSON({ jsonData: pdfDownload[pdf].jsonData , fileName: pdfDownload[pdf].fileName });
                  await generatePdfFromWebPage({ pageRefs: pdfDownload[pdf]?.pageRefs, fileName: pdfDownload[pdf].fileName, heading: pdfDownload[pdf].heading});
              });
